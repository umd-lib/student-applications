namespace :db do
  ...
  task verify_resume_attachments: :environment do
    ...
    puts "Found #{resumes_in_storage.count} files in '#{RESUME_STORAGE_DIR}' storage directory"

    resume_file_attachments = Resume.all.map(&:file).map(&:path)
    puts "Found #{resume_file_attachments.count} file attachments in database."
    ...
    if missing_files.count == 0
      puts "All file attachments found."
    else
      issues_found = true
      puts "There are #{missing_files.count} missing files."
    ...
    if orphaned_files.count == 0
      ...
      issues_found = true
      puts "There are #{orphaned_files.count} orphaned files (files without an associated Resume record in the database)"

...
(2016..2020).each_with_index do |yr, i|
  %w(Dec May).each_with_index { |m, ii| Enumeration.find_or_create_by(value: "#{yr} #{m}", list: Enumeration.lists['graduation_year'] )}
  %w(Spring Fall).each_with_index { |s, ii| Enumeration.find_or_create_by(value: "#{s} #{yr}", list: Enumeration.lists['semester'] )}
end
